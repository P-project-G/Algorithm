...
def move(v,h,vertical,horizon):
    c = 0
    while board[v+vertical][h+horizon] != '#' and board[v][h] != 'O':
        v += vertical
        h += horizon
        c += 1
    return v,h,c
            ...
            if board[dbv][dbh] != 'O':
                ...
                if drv == dbv and drh == dbh:
                    if rc > bc:
                        ...
                        dbv -= vertical[k]
                        dbh -= horizon[k]
                if not visit[drv][drh][dbv][dbh]:
                    visit[drv][drh][dbv][dbh] = True
                    queue.append( [drv,drh,dbv,dbh,d+1] )
    return -1
...
2 초	512 MB	42549	12202	6386	25.438%
문제
...
편의상 1×1크기의 칸으로 나누어져 있다. 가장 바깥 행과 열은 모두 막혀져 있고,
보드에는 구멍이 하나 있다. 빨간 구슬과 파란 구슬의 크기는
보드에서 1×1크기의 칸을 가득 채우는 사이즈이고, 각각 하나씩 들어가 있다.
