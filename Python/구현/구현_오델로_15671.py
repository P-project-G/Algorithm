...
def move(x,y,bw,xx,yy):
    bwarr = []
    while True:
        #검은돌이 흰돌을 잡는 경우
        if board[x][y] == 'W' and bw==0:
            bwarr.append([x,y])
            x += xx
            y += yy
            if x<0 or y<0 or x>5 or y>5:
                return 0
        elif board[x][y] == 'B' and bw==0:
            return bwarr

        #흰돌이 검은돌을 잡는 경우
        if board[x][y] == 'B' and bw==1:
            bwarr.append([x,y])
            x += xx
            y += yy
            if x<0 or y<0 or x>5 or y>5:
                return 0
        elif board[x][y] == 'W' and bw==1:
            return bwarr
...
board = [['.' for _ in range (6)] for _ in range (6)] # 6*6 오델로판
board[2][2], board[3][3] = 'W','W' # 백돌 초기값
board[2][3], board[3][2] = 'B','B' # 흑돌 초기값

n = int(input()) # 게임 로그의 수 n
log = [] # 로그를 받아들일 배열
for i in range (n):
...
xx,yy = [1,-1,0,0,1,-1,-1,1],[0,0,1,-1,1,-1,1,-1]
cnt = 0
answer = []
while log:
    x,y = log.pop(0)
    ...
    for k in range (8):
        ...
        if dx<0 or dy<0 or dx>5 or dy>5:
            continue
        if board[dx][dy] == 'W' and cnt%2 == 0: # 찾은 것이 흰돌이고 검은돌 턴일 경우
            temp = move(dx,dy,cnt%2,xx[k],yy[k])
            while temp:
                bx,by=temp.pop()
                board[bx][by] = 'B'

        if board[dx][dy] == 'B' and cnt%2 == 1: # 찾은 것이 검은돌이고 흰돌 턴일 경우
            temp = move(dx,dy,cnt%2,xx[k],yy[k])
            while temp:
                wx,wy=temp.pop()
                board[wx][wy] = 'W'
    cnt += 1 # 검은돌, 흰돌 턴 판정하기 위함
