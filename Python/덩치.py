...
rank=[]
for i in range(N):
    people.append(list(map(int,input().split()))) # [몸무게, 키] 형태로 people에 저장
for i in range(N):
    k=1 # 1등으로 시작한다.
    for j in range(N): # for문 속에서, i의 키가 j의 키보다 작고, i의 몸무게가 j의 몸무게보다 적을 때 등수를 증가시킨다.
        if(people[i][0]<people[j][0] and people[i][1]<people[j][1]):
            k=k+1
    rank.append(str(k)) # 출력할 때, [ ?, ?, ?, ?, ? ]이 아닌 ?, ?, ?, ?, ?을 만들기 위해 문자열로 추가
print(" ".join(rank)) # join 이용하여 괄호 삭제하여 출력