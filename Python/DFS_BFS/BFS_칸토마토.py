...
tmt_farm = [[list(map(int,input().split())) for _ in range (n)] for _ in range (d)]
vertical=[1,-1,0,0,0,0]
horizon=[0,0,1,-1,0,0]
height=[0,0,0,0,1,-1]

queue=deque()
visit=[[[0]*m for _ in range (n)] for _ in range (d)]
...
while queue:
    ...
    for k in range(6):
        ...
        dy = b+horizon[k]
        dz = c+height[k]
        if n>dx>=0 and m>dy>=0 and d>dz>=0 and tmt_farm[dz][dx][dy]==0 and visit[dz][dx][dy]==0:
            queue.append((dx,dy,dz))
            tmt_farm[dz][dx][dy] = tmt_farm[c][a][b]+1
            visit[dz][dx][dy] = 1
