...
"""
def solution(team_arr,idx):
    global answer
    start_value, link_value = 0, 0
    if len(start) == N/2:
        result_link = set(link) - set(start)
        for i in start:
            for j in start:
                start_value += team_arr[i][j]

        for i in result_link:
            for j in result_link:
                link_value += team_arr[i][j]
        compare = abs(start_value - link_value)
        if answer > compare:
            answer = compare
        return
    ...
    for i in range (idx,N):
        ...
        if len(start) <= N/2:
            start.append(i)
            switch=solution(team_arr,idx+1)
            start.pop()

answer=10000000
N=int(input())
team_arr=[list(map(int, input().split())) for i in range (N)]
start=[]
link= range(N)
solution(team_arr,0)
print(answer)