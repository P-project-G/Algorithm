...
주어진 연산자가 덧셈(+) 2개, 뺄셈(-) 1개, 곱셈(×) 1개, 나눗셈(÷) 1개
인 경우에는 총 60가지의 식을 만들 수 있다.
...
2 초	512 MB	31130	16654	10215	49.800%
"""



def solution(answer,p,m,mul,div,d):
    ...
    if d == N :
        ...
        return
    if p > 0:
        solution(answer+A[d],p-1,m,mul,div,d+1)

    if m > 0:
        solution(answer-A[d],p,m-1,mul,div,d+1)

    if mul > 0:
        solution(answer*A[d],p,m,mul-1,div,d+1)

    if div > 0:
        if answer>=0:
            solution(int(answer/A[d]),p,m,mul,div-1,d+1)
        if answer<0:
            solution(-(int(abs(answer)/A[d])),p,m,mul,div-1,d+1)
if __name__ == '__main__':
    ...
    solution(A[0],operator[0],operator[1],operator[2],operator[3],1)
    print(max_result)
    print(min_result)